...
A linear regression model $Y = X^\top\beta + \eps$ is a special case where $g(X) = X^\top \beta$, and thus its optimality hinges on whether the funcitonal-form assumption $\E[Y \mid X] = X^\top\beta$ is satisfied or not.
...
For another example, if the range of a dependent variable is restricted to $(0,1)$, a logistic regression model $g(X, \theta) = \exp(X^\top \theta)/[1 + \exp(X^\top \theta)]$ is popularlyF used. 
...
Re-ordering the observations in the increasing order of $|X_i - x|$, so that $|X^{(1)} - x| \le |X^{(2)} - x| \le \cdots \le |X^{(n)} - x|$, estimate $g_0(x)$ by
\begin{align*}
...
The idea of kernel regression is similar to that of $k$NN regression, but differs in that it introduces a \textbf{bandwidth\index{bandwidth}} parameter $h$ such that only the observations whose $X_i$ falling into the interval $[x-h, \; x+h]$ are used to compute the estimator.
...
From a sraightforward calculation, one can find that this $\hat g^{const}_n(x)$ is equivalent to the solution of the following weighted least squares problem:
\begin{align*}
...
For example, letting $u_j = (X_{i,j} -x_j)/h$ for a bandwith $h$ common to all $j$, the product uniform kernel function is
\begin{align*}
...
This suggests that if $g_0(\cdot)$ is sufficiently smooth, we can approximate $g_0(x)$ by $g_0(x) \approx p(x)^\top \theta_0$, where $p(x) = (1, x, x^2, \ldots, x^k)^\top$, the \textbf{power series} of order $k$, and $\theta_0 = (g_0(0), g_0^{(1)}(0), \ldots, g_0^{(k)}(0)/k!)^\top$.
...
\begin{align*}
	...
	& = p(X)^\top\theta_0 + \eta
\end{align*}
where $\eta \equiv v(k) + \eps$, and $v(k) \equiv g_0(x) - p(X)^\top\theta_0$.
...
Thus, if $k$ grows sufficiently slowly such that $k/\sqrt{n} \to 0$ is satisfied, we obtain $\hat g_n(x) - g_0(x) \overset{p}{\to} 0$ for any $x \in \mathcal{X}$, as desired.
...
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize, frame=single]
# Kernel regression #
 ghat_kernel <- npreg(food ~ inc)
\end{lstlisting}

Next, we perform the series regression.
The B-spline basis function can be computed using the \textbf{bs} function.
Here, we set the 25\%, 50\%, and 75\% empirical quantiles of \texttt{inc} as the knot values and create B-splines with $k = 6$.
...
To summarize the estimation results of nonparametric regression, they should be presented visually.
...
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize, frame=single]
 ...
 par(new = T)
 plot(inc[order(inc)], ghat_kernel$mean[order(inc)], xlab = "", ylab = "",
	xlim = range(inc), ylim = range(food), type = "l", lwd = 2, col = 2)
 par(new = T)
 plot(inc[order(inc)], ghat_spline[order(inc)], xlab = "", ylab = "",
