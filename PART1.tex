...
In contrast, if one assumes a specific functional form $g(X, \theta)$ for $\E[Y \mid X]$ with a finite dimensional parameter vector $\theta$, this approach is \textbf{parametric\index{parametric}}.
...
Dividing both the numerator and the denominator by $nh$ is for technical convenience.
...
\end{align*}
Recalling that $\int K(u)\text{d}u = 1$ and assuming that $\E[ Y \mid X = \cdot]$ and $f_X (\cdot)$ are bounded continuous functions, as $h \to 0$, we have 
\begin{align*}
	\int K(u) \E[ Y \mid X = x + uh] f_X (x + uh)\text{d}u \to \E[ Y \mid X = x] f_X (x)
\end{align*}
...
Thus, if $nh \to \infty$, by Chebyshev's inequality, we get $(nh)^{-1} \sum_{i = 1}^n K\left( \frac{X_i - x}{h}\right) Y_i \overset{p}{\to} \E[ Y \mid X = x] f_X (x)$.

By the same argument, we can prove that the denominator of $\hat m^{const}_n(x)$, $(nh)^{-1} \sum_{i = 1}^n K\left( \frac{X_i - x}{h}\right)$, converges to $f_X(x)$ in probability (this is the so-called \textbf{kernel density estimator\index{kernel density estimator}}) as $h \to 0$ and $nh \to \infty$.
...
As we have discussed, $h$ must converge to zero sufficiently slowly so that $nh$ can increase to infinity.
...
\end{align*}
where $\lambda(k)$ is a constant depending on $k$ and the choice of the basis function (and also the dimension of $X$) satisfying $\lambda(k) \to 0$ as $k \to \infty$.
...
Indeed, we can estimate $\theta_0$ simply by running an OLS regression of $Y$ on $p(x)$:\footnote{
	...
	However, noting that the dimension of this matrix grows to infinity, it is generally possible that the matrix is not invertible for finite $n$.
...
\end{align*}
Thus, compared to the kernel regression approach, series regression is much easier to implement.\footnote{
	However, deriving local asymptotic properties is more difficult for series regression than for kernel regression.
	}
...
\end{align*}
Then, since we have assumed $O(\lambda(k)) = o(1)$, it suffices to show that $|p(x)^\top (\hat \theta_n - \theta_0)| = o(1)$.
Here, assuming that the basis functions are bounded, we have $||p(x)|| = (p_1^2(x) + \cdot + p_k^2(x))^{1/2} = O(\sqrt{k})$.
