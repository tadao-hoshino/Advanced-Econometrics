...
\begin{align}\label{eq:linmodel}
	Y = X^\top \beta_0 + \eps, 
\end{align}
where $Y$ is a dependent variable, $X$ is a vector of explanatory variables, and $\eps$ is an unobserved random variable (i.e., error term) satisfying $\E[\eps | X] = 0$.
...
Here, note that the conditional expectation function $\E[Y | X]$ is a minimizer of the \textbf{mean squared error\index{MSE}} (MSE) among all possible regression functions:
\begin{align}\label{eq:msemin}
	\E[Y|X] = \argmin_{g \in \mcl{G}} \E[(Y - g(X))^2],
\end{align}
where $\mcl{G}$ is the set of all measurable functions of $X$.
Since we have assumed that $\E[\eps | X] = 0 $ ($\iff \E [Y|X] = X^\top \beta_0$), we can restrict the above MSE minimization with respect to the class of linear functions of $X$:
\begin{align*}
	\beta_0 
	& = \argmin_{\beta \in \mbb{R}^{\text{dim}(X)}} \E[(Y - X^\top \beta)^2]\\
	& = \E[XX^\top]^{-1} \E[XY].
\end{align*} 
Thus, we can see that the OLS objective function in \eqref{eq:ols} is the sample analog of the population MSE, and that the estimated regression function, say $\hat g_n(X) \equiv X^\top \hat \beta_n^{ols}$, is an estimator of $\E[Y|X]$.\footnote{\label{foot:ols_moment}
	Another interpretation of the OLS estimator is that it is a sample solution to the moment equation $\E[Y | X] = X^\top\beta$.
	Premultiplying $X$ on both sides of this equation and taking the expectation yield
	\begin{align*}
		\E[X \E[Y|X]] = \E[XX^\top]\beta_0
...
Suppose that $\E[\eps^2 | X ] = \sigma^2 < \infty$ (i.e., \textbf{homoskedasticity\index{homoskedasticity}}) for simplicity.
Write
\[
...
\end{exercise}
\begin{exercise}\upshape
	Suppose now that the error term is heteroskedastic in the following sense: $\E[\eps^2 | X] = \sigma^2(X)$.
...
\begin{align*}
	...
	&\text{CATE (the conditional average treatment effect)}: \E[Y(1) - Y(0) | X = x] \\
	&\text{ATET (the average treatment effect on the treated)}: \E[Y(1) - Y(0) | D = 1].
\end{align*}
...
\begin{align*}
	& \Pr(\text{Always-taker} | Z = 1) = \Pr(\text{Always-taker} | Z = 0) = \pi_A\\
	& \Pr(\text{Never-taker} | Z = 1) = \Pr(\text{Never-taker} | Z = 0) = \pi_N\\
	& \Pr(\text{Complier} | Z = 1) = \Pr(\text{Complier} | Z = 0) = \pi_C.
\end{align*}
Under these conditions, one can show that
\begin{align}\label{eq:LATE}
\begin{split}
	\underset{\text{\bf LATE}}{\E[Y(1) - Y(0) | \text{Complier}]} 
...
\small{\begin{align*}
	\E[Y |Z = 1 ] 
	& = \E[Y |Z = 1, \text{AT} ] \cdot \Pr(\text{AT}| Z = 1) + \E[Y | Z = 1, \text{NT} ] \cdot \Pr(\text{NT}|Z = 1) + \E[Y | Z = 1, \text{C} ] \cdot \Pr(\text{C}|Z = 1)\\
	& = \E[Y(1) |Z = 1, \text{AT} ] \cdot \pi_A + \E[Y(0) | Z = 1, \text{NT} ] \cdot \pi_N + E[Y(1) | Z = 1, \text{C} ] \cdot \pi_C\\
	& = \E[Y(1) | \text{AT} ] \cdot \pi_A  + E[Y(0) | \text{NT} ] \cdot \pi_N  + E[Y(1) | \text{C} ] \cdot \pi_C,
\end{align*}}
\normalsize
...
\begin{align*}
	\E[Y | Z = 0]
	& = \E[Y(1) | \text{AT} ] \cdot \pi_A + \E[ Y(0) | \text{NT} ] \cdot \pi_N  + \E[ Y(0) | \text{C} ]  \cdot \pi_C
\end{align*}
Thus, 
\begin{align*}
	\E [ Y |Z = 1 ] - E[ Y | Z = 0 ] 
	&= \left\{ \E[ Y(1)| \text{Complier} ] - \E[ Y(0) | \text{Complier} ] \right\} \cdot \pi_C\\
	& = \mbf{LATE}\cdot\pi_C
\end{align*}
...
\hrulefill
\begin{exercise}\upshape
	Prove the following equality: $\displaystyle \frac{\Cov(Z, Y)}{\Cov(Z, D)} = \frac{\E[Y | Z = 1] - \E[Y | Z = 0]}{\E[D | Z = 1] - \E[D | Z = 0]}$.
\end{exercise}
...
\end{align*}
Since $S$ and $S^*$ are observationally equivalent ($F^1_{Y,X} = F^*_{Y,X}$), we have $\E_1[Y |X] = \E^*[Y |X]$ almost surely (a.s.), which implies
...
As shown in \eqref{eq:msemin},  in terms of minimization of MSE, the most optimal regression function is the conditional expectation function $\E [Y | X]$.
Thus, the validity of linear regression hinges on whether the funcitonal-form assumption $\E[Y | X] = X^\top\beta$ is true for some $\beta$ or not.
In reality, however, it would be extremely rare that data follow such a linear relationship exactly, and if the functional-form assumption is not met, what the linear regression produces is only a ``linear approaximation" of $\E[Y | X]$.
...
Thus, ideally, we would like to estimate $\E [Y | X]$ without imposing any a priori functional-form assumptions.
Such a regression approach is called \textbf{nonparametric\index{nonparametric}}.
In contrast, if one assumes a specific functional form $g(X, \theta)$ for $\E[Y|X]$ with a finite dimensional parameter vector $\theta$, this approach is \textbf{parametric\index{parametric}}.
...
The intermediate case between the nonparametric and parametric models is called \textbf{semiparamtric\index{semiparametric}}.
As explained below, estimating a fully nonparametric regression is almost infeasible in practice due to the curse of dimensionality unless the dimension of $X$ is less than two or three.
On the other hand, assuming a full parametric specification is restrictive.
...
\begin{align*}
	Y = m(X) + \eps.
\end{align*}
Here, we assume that $\E[\eps | X] = 0$ (i.e., $X$ is exogenous) so that $m(X) = \E[Y|X]$.
...
This type of nonparametric regression is particuarly called \textbf{series regression\index{series regression}} estimator or \textbf{sieve method\index{sieve method}}.
...
(There is a bias-variance trade-off in terms of the choice of $k$.)
...
The idea of kernel regression is similar to that of $k$NN regression, but differs in that it specifies a \textbf{bandwidth\index{bandwidth}} $h$ such that only the observations whose $X_i$ falling into the interval $[x-h, \; x+h]$ are used to compute the estimator.
For example, the \textbf{local constant regression\index{local constant regression}} estimator is defined as
\begin{align*}
...
Specifically, we need to introduce a ``multivariate'' kernel weighting function, say $\mathbf{K}(u)$, where $u = (u_1, \ldots, u_d) \in \mathbb{R}^d$.
One simple way to construct $\mathbf{K}(u)$ is to take the product of one-dimensional kernel functions: $\mathbf{K}(u) = \prod_{j=1}^d K(u_d)$.
