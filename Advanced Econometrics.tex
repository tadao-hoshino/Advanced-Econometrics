...
When $\Cov(X, \eps) = 0$ holds true, we say that $X$ is \textbf{exogenous\index{exogeneity}} (in the context of \eqref{eq:simple}).\footnote{
...
\[
	Y(1) - Y(0).
\]
This quantity is referred to as the (individual) \textbf{treatment effect\index{treatment effect}}, and $D$ is called the \textbf{treatment variable\index{treatment variable}} in this context.
...
This is the so-called \textbf{kernel density estimator\index{kernel density estimator}}.
...
\begin{itemize}
	...
	\end{align*}
	where $\sigma$ is a predetermined function (so-called \textbf{activation function}\index{activation function}), and $(\gamma_{01}, \gamma_{11}, \gamma_{02}, \gamma_{12}, \ldots)$ are parameters to be estimated.
...
Then, the idea of \textbf{semiparametric regression}\index{semiparametric regression} is to decompose $g_0(X)$ into a parametric part and a nonparametric part, so that we can retain certain nonparametric flexibility without losing computational feasiblity.
...
\textbf{Partially linear model}\index{partially linear model} is a regression model that consists of parametric linear component and a nonparametric component in an additive form:
...
Such a model is called the \textbf{functional coefficient model}\index{functional coefficient model}; namely, 
...
In the field of spatial statistics, the functional coefficient model with $X_2 = (\text{longitude}, \text{latitude})$ is specifically referred to as the \textbf{geographically weighted regression (GWR) model}\index{geographically weighted regression model}.
...
Another important class of semiparametric models is the \textbf{generalized additive model}\index{generalized additive model} (GAM).
